...
func (p Point) shift(distance int, dir Dir) Point {
	switch dir {
	case Horizontal:
		return Point{
			x: p.x + distance,
...
func (c Char) shift(distance int, dir Dir) Char {
	return Char{
		root:         c.root.shift(distance, dir),
		p:            c.p.shift(distance, dir),
		c:            c.c,
		d:            c.d,
		master:       c.master,
...
func (s String) shift(distance int, dir Dir) String {
	return String{
		root: s.root.shift(distance, dir),
		s:    s.s,
		d:    s.d,
	}
}
...
func (g *Generator) checkValidation(s string, x int, p Point, d Dir) (bool, []Point, Point) {
	...
	case Vertical:
		...
		if p.y >= x {
			...
			if abs(p.y) + g.height > g.maxHeight {
				return false, nil, Point{}
			}
		}
		root := Point{
...
func (g *Generator) shift(p Point, d Dir) {
	loc := make(map[Point]Char, 0)
	los := make(map[Point]String, 0)
	sol := make(map[String]Point, 0)
	shift := 0
	switch d {
	case Horizontal:
		if p.x >= 0 {
	...
	for key, value := range g.locationOfChars {
		loc[key.shift(shift, d)] = value.shift(shift, d)
	}
	for key, value := range g.locationOfStrings {
		los[key.shift(shift, d)] = value.shift(shift, d)
	}
	for key, value := range g.stringOfLocations {
		sol[key.shift(shift, d)] = value.shift(shift, d)
	}
	g.locationOfStrings = los
	g.locationOfChars = loc
	g.stringOfLocations = sol
}
...
func Start(data string, desireSize, maxWidth, maxHeight, wordsSize, interactions int) {
